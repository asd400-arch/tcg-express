...
## 4부. 즉시 크레딧 (선인정·후검증) — 스캇님 정책 승인 (2026-08-24)

**요구:** 고객 충전은 결제 즉시 완료돼야 함 (드라이버 출금은 추후 일괄 정산 가능).
**제약:** DBS 입금 알림 이메일은 실시간이 아님 (실측 최대 ~1시간 지연). 직접 수취 PayNow에는 실시간 신호가 없음.
**해법:** 선인정·후검증. 승인된 정책: **건당 S$500까지 즉시 인정.**

### 새 흐름
1. 고객이 고유 금액 QR로 이체 → **"I've paid" 버튼** 탭
2. S$500 이하: **즉시 지갑 크레딧 + 보너스 + "충전 완료" 화면** (topup 상태 pending→processing)
3. S$500 초과: "은행 확인 대기 중" 안내, 확인되는 대로 자동 크레딧 (폴링 유지)
4. 은행 알림 도착(수십 분 내) → 자동 대사가 processing 건을 **검증 완료 처리** (이중 크레딧 없음, 48시간까지 매칭)
5. 은행 알림이 끝내 안 오는 건 → processing 상태로 어드민 화면에 남음 (어드민 목록에서 status=processing 필터로 확인) → 스캇님이 회수/문의 결정. 자동 회수는 하지 않음.

### 변경 파일
- `lib/walletService.ts` — `claimPayNowTopup`(즉시 크레딧, 이중탭 방지, 실패 시 롤백), 자동대사·어드민 확인이 processing 건은 검증만 하도록 수정, 고유금액 배정이 processing 금액도 예약 유지
- `types/wallet.ts` — `INSTANT_TOPUP_LIMIT: 500`
- `app/api/wallet/topup/claim/route.ts` — **새 파일**
- `app/components/wallet/TopupModal.tsx` — "I've paid" 버튼, 한도 초과 시 대기 안내

### 리스크 통제
- 미검증 노출은 건당 최대 S$500, 보너스는 중복 지급 가드 있음
- B2B 고객 신원 확보 + 배송 보류 가능 → 회수 실효성 높음
- 한도 변경은 `types/wallet.ts`의 INSTANT_TOPUP_LIMIT 한 줄
