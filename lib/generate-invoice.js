...
export async function generateInvoice(jobId) {
    ...
    for (const [label, url] of photoEntries) {
      try {
        ...
        if (imgRes.ok) {
          ...
          y += 14;
          // Check page space
          if (y + 160 > 750) { doc.addPage(); y = 50; }
          doc.image(imgBuf, 50, y, { width: 200 });
          y += 160;
        }
      } catch {
  ...
  if (job.customer_signature_url) {
    ...
    try {
      ...
      if (sigRes.ok) {
        ...
        doc.image(sigBuf, 50, y, { width: 180 });
        y += 80;
      }
    } catch {
  ...
  if (uploadErr) {
    console.error('Invoice upload error:', uploadErr.message);
  ...
  try {
    ...
    // Non-fatal: notification/email failure shouldn't break invoice gen
  }
