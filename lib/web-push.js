...
export async function sendPushToUser(userId, { title, body, url, data, channelId }) {
  ...
  if (expoSubs.length === 0) {
    console.warn(`[web-push] userId=${userId}: ${subs.length} sub(s) found, 0 expo — no expo push sent`);
  }
  ...
  if (expoSubs.length > 0) {
    try {
      console.log(`[web-push] expo push userId=${userId} tokens: ${expoSubs.map(s => s.expo_token?.slice(0, 20)).join(', ')}`);
