...
function generatePayNowQR(amount) {
  ...
  const payload =
    ...
    field('54', amountStr) +
    field('58', 'SG') +
    // NOTE: keep in sync with the bank-side PayNow registry name.
    // Still 'HHI Solutions Pte Ltd' until DBS processes the ACRA name
    // change — banking apps show the registry name, and a mismatch with
    // the app UI makes customers abort. Flip to 'Tech Chain Global Pte
    // Ltd' (exactly 25 chars) once DBS confirms the update.
