...
function generatePayNowQR(amount) {
  ...
  const paynowFields =
    ...
    field('01', '2') +
    field('02', '202005872W') +
    field('03', '0') +  // amount NOT editable — exact-amount matching depends on this
    field('04', expiry);
  ...
  const payload =
    ...
    field('54', amountStr) +
    field('58', 'SG') +
    // Keep in sync with the bank-side PayNow registry name — banking apps
    // show the registry name, and a mismatch with the app UI makes
    // customers abort. DBS completed the name change on 2026-08-30 and now
    // shows 'TECH CHAIN GLOBAL PTE LTD' (25 chars, the EMVCo field limit).
    // walletService.ts recipient_name must match this exactly.
    field('59', 'TECH CHAIN GLOBAL PTE LTD') +
    field('60', 'Singapore') +
    '6304';
