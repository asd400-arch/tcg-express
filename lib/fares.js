...
export function checkVehicleFit(driverVehicleKey, jobVehicleRequired) {
  ...
  // If driver has no vehicle set, block (shouldn't happen)
  if (!driverVehicleKey) return { ok: false, required: jobVehicleRequired, driverVehicle: 'none' };

  const driverIdx = getVehicleModeIndex(driverVehicleKey);
  const jobIdx = getVehicleModeIndex(jobVehicleRequired);

  // If either is unknown/special, allow
