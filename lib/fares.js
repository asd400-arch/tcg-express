...
export const SIZE_TIERS = [
  { key: 'envelope', label: 'Envelope / Document', baseFare: 6, maxWeight: 1, icon: '✉️' },
  { key: 'small', label: 'Small (< 5kg)', baseFare: 10, maxWeight: 5, icon: '📦' },
  { key: 'medium', label: 'Medium (5–20kg)', baseFare: 20, maxWeight: 20, icon: '📦' },
  { key: 'large', label: 'Large (20–50kg)', baseFare: 40, maxWeight: 50, icon: '🏋️' },
  { key: 'bulky', label: 'Bulky / Heavy (50kg+)', baseFare: 80, maxWeight: Infinity, icon: '🚛' },
];
...
export const URGENCY_MULTIPLIERS = {
  ...
  express: { label: 'Express', multiplier: 1.3, desc: 'Within 4 hours' },
  urgent: { label: 'Urgent', multiplier: 1.6, desc: 'Within 1 hour' },
};

export const ADDON_OPTIONS = [
  { key: 'extra_manpower', label: 'Extra Manpower', price: 25, unit: 'person', icon: '👷', hasQty: true },
  { key: 'white_glove', label: 'White Glove Service', price: 40, unit: 'flat', icon: '🧤', hasQty: false },
  { key: 'stairs', label: 'Stairs Surcharge', price: 10, unit: 'floor', icon: '🪜', hasQty: true },
];

export const BASIC_EQUIPMENT = [
  { key: 'trolley', label: 'Trolley', price: 0, icon: '🛒' },
  { key: 'wrapping', label: 'Wrapping', price: 15, icon: '📦' },
  { key: 'dismantlement', label: 'Dismantlement', price: 0, icon: '🔧' },
  { key: 'installation', label: 'Installation', price: 0, icon: '🔩' },
];

export const SPECIAL_EQUIPMENT = [
  { key: 'pallet_jack', label: 'Pallet Jack', price: 50, icon: '🏗️' },
  { key: 'lift_truck', label: 'Lift Truck', price: 100, icon: '🚛' },
  { key: 'crane', label: 'Crane', price: 150, icon: '🏗️' },
...
// ── Vehicle Modes (11 types) ──
// Launch pricing (24 Sep 2026): base fares lowered ~30%; bicycle / e-bike removed
// (B2B tech-equipment focus — motorcycle covers small parcels).
export const VEHICLE_MODES = [
  { key: 'motorcycle', label: 'Motorcycle', maxWeight: 8, maxL: 40, maxW: 30, maxH: 30, baseFare: 6, icon: '🏍️' },
  { key: 'car', label: 'Car', maxWeight: 20, maxL: 70, maxW: 50, maxH: 50, baseFare: 11, icon: '🚗' },
  { key: 'mpv', label: 'MPV', maxWeight: 50, maxL: 110, maxW: 80, maxH: 50, baseFare: 16, icon: '🚙' },
  { key: 'van_1_7m', label: '1.7m Van', maxWeight: 400, maxL: 160, maxW: 120, maxH: 100, baseFare: 30, icon: '🚐' },
  { key: 'van_2_4m', label: '2.4m Van', maxWeight: 800, maxL: 230, maxW: 120, maxH: 120, baseFare: 42, icon: '🚐' },
  { key: 'lorry_10ft', label: '10ft Lorry', maxWeight: 1200, maxL: 290, maxW: 140, maxH: 170, baseFare: 65, icon: '🚚' },
  { key: 'lorry_14ft', label: '14ft Lorry', maxWeight: 2000, maxL: 420, maxW: 170, maxH: 190, baseFare: 95, icon: '🚚' },
  { key: 'lorry_24ft', label: '24ft Lorry', maxWeight: 4000, maxL: 720, maxW: 220, maxH: 220, baseFare: 190, icon: '🚛' },
  { key: 'trailer_20ft', label: '20ft Trailer', maxWeight: 10000, maxL: 600, maxW: 240, maxH: 260, baseFare: 150, icon: '🚛' },
  { key: 'trailer_40ft', label: '40ft Trailer', maxWeight: 20000, maxL: 1200, maxW: 240, maxH: 260, baseFare: 280, icon: '🚛' },
...
export function normalizeVehicleKey(key) {
  ...
  return LEGACY_VEHICLE_MAP[lower] || lower;
}
...
export function checkVehicleFit(driverVehicleKey, jobVehicleRequired) {
  ...
  if (!driverVehicleKey) return { ok: false, reason: 'no_vehicle', required: jobVehicleRequired, driverVehicle: 'none' };
  ...
  // Motorcycle jobs are small-vehicle exclusive:
  // only bicycle, e_bike, and motorcycle may bid — larger vehicles are rejected
  if (jobKey === 'motorcycle') {
    const motorcycleIdx = getVehicleModeIndex('motorcycle');
    if (driverIdx > motorcycleIdx) {
      const driverMode = VEHICLE_MODES[driverIdx];
      return {
        ok: false,
        reason: 'exclusive',
        required: 'Motorcycle or smaller (small vehicle jobs are exclusive)',
...
// ── Launch driver top-up (TCG-funded) ──
// Credited to the driver's wallet (withdrawable) when the customer confirms delivery,
// on top of the full fare. Override / switch off without a deploy via express_settings
// key 'driver_launch_topup' (JSON with the same shape, e.g. {"enabled":false}).
export const DRIVER_LAUNCH_TOPUP = {
  enabled: true,
  until: '2026-10-31T23:59:59+08:00',
  amounts: {
    motorcycle: 2,
    car: 4, mpv: 4,
    van_1_7m: 6, van_2_4m: 6,
    lorry_10ft: 10, lorry_14ft: 10, lorry_24ft: 10, trailer_20ft: 10, trailer_40ft: 10,
  },
  default: 4, // unknown / free-text vehicle types
};

export function getLaunchTopup(vehicleKey, config = DRIVER_LAUNCH_TOPUP, now = Date.now()) {
  if (!config || !config.enabled) return 0;
  if (config.until && new Date(config.until).getTime() < now) return 0;
  if (!vehicleKey || vehicleKey === 'any') return 0;
  const key = normalizeVehicleKey(vehicleKey);
  const amounts = config.amounts || {};
  const amt = key && amounts[key] != null ? amounts[key] : config.default;
  return Number(amt) || 0;
}
...
export const DISTANCE_PER_KM = 1.0;

// P6 manpower rules (weight kg + volume m³):
//   1 person : ≤20 kg AND <1 m³
//   2 persons: (≤20 kg AND ≥1 m³) OR 20–40 kg
//   3 persons: >40 kg
// Client may increase above minimum; decreasing below minimum is blocked.
export function getAutoManpower(weightKg, lengthCm = 0, widthCm = 0, heightCm = 0) {
  const w  = parseFloat(weightKg) || 0;
  const l  = parseFloat(lengthCm)  || 0;
  const wi = parseFloat(widthCm)   || 0;
  const h  = parseFloat(heightCm)  || 0;
  const volumeM3 = (l * wi * h) / 1_000_000;

  if (w > 40) return 3;
  if (w > 20) return 2;
  if (volumeM3 >= 1) return 2;
  return 1;
}
...
export function calculateFare({ sizeTier, vehicleMode, urgency = 'standard', addons = {}, distanceKm = 0, basicEquipment = [], basicEquipCount = 0, specialEquipment = [], isEvSelected = false, saveModeDiscount = 0, zoneSurcharge = 0 }) {
  ...
  let specialEquipmentTotal = 0;
  if (specialEquipment.length > 0) {
    for (const eq of SPECIAL_EQUIPMENT) {
      if (specialEquipment.includes(eq.key) && eq.price > 0) {
        specialEquipmentTotal += eq.price;
        addonLines.push({ key: eq.key, label: eq.label, qty: 1, cost: eq.price, unit: 'item', price: eq.price });
      }
    }
  }
  ...
  // SaveMode is mutually exclusive with express/urgent — block server-side regardless of client input
  let saveModeAmount = 0;
  if (saveModeDiscount > 0 && urgency === 'standard') {
    saveModeAmount = parseFloat((baseFare * saveModeDiscount).toFixed(2));
  }

  const zs = parseFloat(zoneSurcharge) || 0;
  const total = baseWithUrgency + addonTotal + equipmentTotal + specialEquipmentTotal + distSurcharge + zs - evDiscount - saveModeAmount;
  return {
    baseFare,
    multiplier,
    baseWithUrgency,
    addonLines,
    addonTotal: addonTotal + equipmentTotal + specialEquipmentTotal,
    distSurcharge,
    zoneSurcharge: zs,
    evDiscount,
    ...
    budgetMax: Math.round(total * 1.3),
  };
}
