 ...
 */
export async function notify(userId, { type, category, title, message, referenceId, emailTemplate, emailData, url, data, inApp = true, push = true, channelId }) {
  ...
  if (push && catPrefs.push) {
    ...
    await sendPushToUser(userId, { title, body: message, url: url || '/', data, channelId }).catch((err) => {
