 ...
 */
export async function notify(userId, { type, category, title, message, referenceId, emailTemplate, emailData, url, data }) {
  ...
  if (catPrefs.push) {
    ...
    sendPushToUser(userId, { title, body: message, url: url || '/', data }).catch((err) => {
